...
※※ 탭 루트입니다. Figma 하단 탭 바는 4개(홈 · 오늘의 일지 · 개선책 · MY)이고
**MY만 아직 화면이 없어** Figma 그대로 보이기만 하고 눌러도 아무 일도 없습니다.

개선책 메인의 "맞춤 영양제 / 주간 리포트" 카드와 "한달 뒤 내 모습" 카드는 아직
눌러도 반응하지 않습니다 — 해당 화면들이 아직 없습니다.
...
- 입력 검증은 **다음 버튼 비활성화까지만** 있습니다. 필수 항목·비밀번호 일치·
  실제 존재하는 날짜를 확인하지만, **왜 막혔는지 알려주지 못합니다** — 디자인에
  입력 필드의 에러 상태가 없습니다. 비밀번호 강도는 서버 규칙을 몰라서
  검사하지 않습니다 (백로그 19번).
