...
Figma 를 전수 재대조한 결과입니다. **이 표가 캐시와 어긋난 부분입니다.**

| 무엇 | 노드 | 상태 |
|---|---|---|
| `Diary_Status` (신규 컴포넌트) | `725:1813` — 변이 `Good`/`Warning`/`Danger` | ✅ 반영 (`ui/diary-status.tsx`) |
| 일지/메인 지난 기록 | `725:1874` (Frame 35) + `Diary_Status` 인스턴스 5개 | ✅ 반영 |
| 나의 LifeDNA 정보 카드 → **탭 구조** | `725:1213`·`725:1294`·`725:1375`·`725:1456`·`726:1472` | ⬜ 미반영 |
| 마이페이지 · 데이터 개인정보 | `583:913` — 빈 화면이었다가 완성됨 | ⬜ 미반영 |
| 마이페이지 · 구독관리 | `585:1399` — 빈 화면이었다가 완성됨 | ⬜ 미반영 |
| `Logo` (신규 컴포넌트, 213×213) | `730:1894`, 섹션 `730:1893` 로고 | ✅ 앱 아이콘·스플래시 반영 (원본 426px — 1024px 마스터 필요) |
| 마이페이지/메인 이름 변경 | `583:969` — `Frame 28` → `마이페이지/메인` | 이름만 |

**컴포넌트 섹션이 둘 늘었습니다** — `725:1677` 오늘의일지(`Diary_Status`),
`730:1893` 로고(`Logo`).

### 나의 LifeDNA 정보 — 탭 구조로 재설계 (미반영)

`DNAKind` 칩 5개가 **탭**이 됐습니다. 선택된 탭만 자기 등급 색(예: 신체
`#DCF7EF`/`#00A172`)을 쓰고 나머지는 기본(흰 배경 `#7A7A7A`). 탭 순서는
**신체 · 정신 · 감정 · 사회 · 환경**으로, 기존 코드의 `신체 · 정신 · 환경 ·
감정 · 사회`와 다릅니다. 칩은 30×11, radius 3, x=11/43/75/107/139, y=28.

탭마다 `LifeDNA_WeeklyInfo_Card` 두 장이 (11,48)·(11,118)에 놓입니다:

| 영역 | 카드 1 (좋음·High) | 카드 2 (주의·Mid) |
|---|---|---|
| 신체 | 수면 패턴(시간·질) | 수면 리듬(크로노타입) |
| 정신 | 스트레스 회복력 | 집중 · 디지털 부하 |
| 감정 | 기분 안정도 | 기분 회복 탄력 |
| 사회 | 사람 만나는 주기 | 사회적 지지감 |
| 환경 | 날씨 영향 | 수면 환경(빛·소음) |

**이 디자인은 아직 작업 중입니다** — 붙이기 전에 디자이너 확인이 필요합니다:

- 아이콘이 **신체·정신 탭에만** 있고 감정·사회·환경은 빈 흰 사각형입니다.
  신체 탭은 두 카드가 같은 아이콘(뇌)을 씁니다.
- `725:1294`(정신)에는 **정신 칩 자체가 없습니다** — 자리만 비어 있습니다.
- `726:1472`(환경)는 **신체와 환경 두 개가 동시에 선택 상태**입니다.
- `725:1213`의 배경 사각형이 **396pt**로, 카드 내용(189pt)보다 두 배 큽니다.
